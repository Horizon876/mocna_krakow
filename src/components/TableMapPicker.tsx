  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-8 text-center">
        <h2
          className="text-[2.5rem] font-bold leading-none tracking-tightest sm:text-[3.5rem]"
          aria-label="Zarezerwuj stolik"
        >
          <span className="text-[#2c5ea9]">Z</span>
          <span className="text-[#de3c42]">a</span>
          <span className="text-[#ffde00]">r</span>
          <span className="text-[#00955e]">e</span>
          <span className="text-[#fa8080]">z</span>
          <span className="text-[#2c5ea9]">e</span>
          <span className="text-[#2c5ea9]">r</span>
          <span className="text-[#de3c42]">w</span>
          <span className="text-[#ffde00]">u</span>
          <span className="text-[#00955e]">j</span>{" "}
          <span className="text-[#fa8080]">s</span>
          <span className="text-[#2c5ea9]">t</span>
          <span className="text-[#2c5ea9]">o</span>
          <span className="text-[#de3c42]">l</span>
          <span className="text-[#ffde00]">i</span>
          <span className="text-[#00955e]">k</span>
        </h2>
          ...
          <div className="date-calendar border border-[#dddddd] bg-white px-5 py-5 sm:px-8 sm:py-6">
            <div className="mb-5 flex items-center justify-between gap-4">
              ...
              <p className="flex-1 text-center font-display text-[1.35rem] font-bold tracking-[-0.02em] text-[#333333] sm:text-[1.5rem]">
                {MONTH_NAMES[viewMonth].charAt(0).toUpperCase() +
                  MONTH_NAMES[viewMonth].slice(1)}{" "}
                {viewYear}
              </p>
              <button
                type="button"
            ...
            <div className="mb-3 grid grid-cols-7">
              {WEEKDAY_NAMES.map((day) => (
                <span
                  key={day}
                  className="text-center text-[0.75rem] font-semibold uppercase tracking-[0.04em] text-[#888888]"
                >
                  {day}
                </span>
              ))}
            ...
            <div className="grid grid-cols-7 gap-y-1">
              {calendarDays.map((day, idx) => {
                ...
                let baseClass =
                  "h-10 w-full text-center text-[0.95rem] leading-10 transition-colors ";
                if (day.isDisabled) {
                  baseClass +=
                    "cursor-not-allowed pointer-events-none font-normal text-[#c8c8c8] ";
                } else {
                  baseClass +=
                    "cursor-pointer font-medium text-[#333333] hover:text-[#2c5ea9] ";
                }

                if (selectedDate === day.iso) {
                  baseClass =
                    "h-10 w-full text-center text-[0.95rem] leading-10 font-bold text-[#2c5ea9] cursor-pointer ";
        ...
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-lg font-bold capitalize text-[#333333]">
            ...
            <button
              ...
              onClick={() => {
                ...
                document.dispatchEvent(
                  new CustomEvent("bookingReset", { bubbles: true }),
                );
              }}
            >
              ← Zmień datę
          ...
          <div className="mb-8 grid grid-cols-4 gap-2 sm:grid-cols-5">
            {TIME_SLOTS.map((slot) => {
              ...
              let btnClass =
                "border px-2 py-2.5 text-[0.875rem] transition-colors ";
              if (!available) {
                btnClass +=
                  "cursor-not-allowed border-transparent bg-transparent font-normal text-[#8a8a8a] opacity-60";
              } else if (isSelected) {
                btnClass +=
                  "cursor-pointer border-[#2c5ea9] bg-[#2c5ea9] font-bold text-white";
              } else {
                btnClass +=
                  "cursor-pointer border-[#e7e6e4] bg-white font-bold text-[#333333] hover:border-[#2c5ea9]/35 hover:bg-[#2c5ea9]/[0.06] hover:text-[#2c5ea9]";
              }
