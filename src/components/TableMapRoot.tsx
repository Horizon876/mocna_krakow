...
export default function TableMapRoot({
  ...
  const [occupiedTables, setOccupiedTables] = useState<Set<string>>(
    new Set(initialOccupiedTables),
  );

  useEffect(() => {
    if (step === "map" && selectedDate && selectedTime) {
      fetch(
        `/api/occupied-tables?date=${encodeURIComponent(selectedDate)}&time=${encodeURIComponent(selectedTime)}`,
      )
        .then((r) => r.json())
        .then((ids) => {
  ...
  return (
    <div className="w-full">
      <input
        type="hidden"
        id="selected-table-input"
        name="tableId"
        value={selectedTableId || ""}
      />
      <input
        type="hidden"
        id="selected-time-input"
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-400">
          ...
          <div className="mb-4 flex flex-wrap gap-4 text-sm font-medium text-[#4b4b4b]">
            <span className="flex items-center gap-2">
              <span className="inline-block h-4 w-4 rounded-full bg-[#2c5ea9] shadow-sm"></span>
              Wolny
            </span>
            <span className="flex items-center gap-2">
              <span className="inline-block h-4 w-4 rounded-full bg-[#9ca3af] shadow-sm"></span>
              Zajęty
            </span>
            <span className="flex items-center gap-2">
              <span className="inline-block h-4 w-4 rounded-full bg-[#de3c42] shadow-sm"></span>
              Wybrany
            </span>
          </div>
          ...
          {selectedTableId && TABLE_META[selectedTableId] && (
            <div className="mt-6 border border-[#e7e6e4] bg-white p-5 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <p className="text-lg font-bold text-[#333333]">
                {TABLE_META[selectedTableId].label}
              </p>
              <p className="text-sm text-[#6b6b6b]">
                {TABLE_META[selectedTableId].zone} ·{" "}
                {TABLE_META[selectedTableId].cap}{" "}
                {TABLE_META[selectedTableId].cap === 1 ? "osoba" : "osoby"}
              </p>
