...
export default function FlyingNavReact() {
  ...
  const isSklepPage =
    currentPath === "/sklep" || currentPath.startsWith("/sklep/");
  ...
  const updateScroll = useCallback(() => {
    ...
    for (const { id, extraOffset } of HOME_SECTIONS) {
      ...
      if (
        ...
        NAV_OFFSET_PX - (extraOffset as number) + 48
      ) {
        active = id;
      }
    }
  ...
  const isGroupActive = (items: { href: string }[]) =>
    items.some((item) => navLinkIsActive(item.href));
  ...
  const navUnderline = (active: boolean) => (
    <span
      className={`absolute inset-x-3 -bottom-px h-[1.5px] bg-[#2c5ea9] transition-opacity duration-200 ${active ? "opacity-100" : "opacity-0"}`}
    ></span>
  );

  const navCtaBase =
    "flex h-[2.125rem] w-auto shrink-0 items-center justify-center gap-1.5 box-border px-2.5 text-[13px] leading-none font-medium tracking-[-0.01em] transition-colors duration-200";
  ...
  return (
    <>
          ...
          style={
            isScrolled
              ? {
                  ...
                  boxShadow: "0 2px 28px -6px rgba(51, 51, 51, 0.14)",
                }
              : {}
          }
        >
          <div className="shell">
            <nav
              className="flex h-16 items-center justify-between gap-4 overflow-visible sm:h-[4.5rem]"
            ...
            >
              ...
              >
                <span
                  className="block h-16 w-[8.5rem] sm:h-[4.5rem] sm:w-[11.5rem]"
                  aria-hidden="true"
                ></span>
                <img
              ...
              {/* Desktop links */}
              <ul
                className="hidden items-center gap-0.5 lg:flex list-none p-0 m-0"
                role="list"
              >
                <li>
                  <a
                    href="/"
                    aria-current={navLinkIsActive("/") ? "page" : undefined}
                ...
                >
                  <button
                    ...
                    onClick={() =>
                      setOpenDropdown(openDropdown === "onas" ? null : "onas")
                    }
                  >
                    O nas
                    {navUnderline(isGroupActive(oNas))}
                    <svg
                      className={`h-3 w-3 transition-transform duration-200 ${openDropdown === "onas" ? "rotate-180" : ""}`}
                      viewBox="0 0 10 10"
                      fill="none"
                      aria-hidden="true"
                    >
                  ...
                  >
                    <div
                      className={`border border-[#333]/[0.07] bg-white py-1 shadow-[0_18px_48px_-14px_rgba(51,51,51,0.2)] transition-opacity duration-200 ${openDropdown === "onas" ? "opacity-100" : "opacity-0"}`}
                      role="menu"
                    >
                      {oNas.map((item) => {
                        ...
                        return (
                          <a
                            ...
                            href={item.href}
                            role="menuitem"
                            className={`block px-3.5 py-2.5 text-[13px] font-medium tracking-[-0.01em] transition-colors duration-150 border-l-2 ${active ? "text-[#2c5ea9] border-[#2c5ea9] bg-[#2c5ea9]/[0.07]" : "text-[#333333] border-transparent hover:bg-[#f4f4f3] hover:border-[#2c5ea9] hover:pl-[0.875rem]"}`}
                          >
                            {item.label}
                          </a>
                        );
                ...
                >
                  <button
                    ...
                    onClick={() =>
                      setOpenDropdown(
                        openDropdown === "oferta" ? null : "oferta",
                      )
                    }
                  >
                    Oferta
                    ...
                    <svg
                      className={`h-3 w-3 transition-transform duration-200 ${openDropdown === "oferta" ? "rotate-180" : ""}`}
                      viewBox="0 0 10 10"
                      fill="none"
                      aria-hidden="true"
                    >
                  ...
                  >
                    <div
                      className={`border border-[#333]/[0.07] bg-white py-1 shadow-[0_18px_48px_-14px_rgba(51,51,51,0.2)] transition-opacity duration-200 ${openDropdown === "oferta" ? "opacity-100" : "opacity-0"}`}
                      role="menu"
                    >
                      {oferta.map((item) => {
                        const active = navLinkIsActive(item.href);
                ...
                <li>
                  <a
                    href="/wydarzenia"
                    aria-current={
                      navLinkIsActive("/wydarzenia") ? "page" : undefined
                    }
                    className={navLinkClass("/wydarzenia")}
                  >
                ...
                <li>
                  <a
                    href="/kawiarnia"
                    aria-current={
                      navLinkIsActive("/kawiarnia") ? "page" : undefined
                    }
                    className={navLinkClass("/kawiarnia")}
                  >
                ...
                <li>
                  <a
                    href="/kontakt"
                    aria-current={
                      navLinkIsActive("/kontakt") ? "page" : undefined
                    }
              ...
              <div className="relative z-20 flex shrink-0 items-center gap-1.5">
                <div className="hidden items-center gap-1.5 sm:flex">
                  ...
                  >
                    {!isSklepPage ? (
                      ...
                      >
                        <path
                          d="M3.5 5h9l-1.25 7.5h-6.5L3.5 5z"
                          stroke="currentColor"
                          strokeWidth="1.25"
                          strokeLinejoin="round"
                        ></path>
                        <path
                          d="M6 5c0-1.1.9-2 2-2s2 .9 2 2"
                          stroke="currentColor"
                          strokeWidth="1.25"
                          strokeLinecap="round"
                        ></path>
                  ...
                  <a
                    ...
                    className={`${navCtaBase} font-semibold text-white hover:shadow-[0_5px_18px_-6px_rgba(44,94,169,0.45)] ${
                      navLinkIsActive("/rezerwacja")
                        ? "bg-[#244d8f]"
                        : "bg-[#2c5ea9] hover:bg-[#244d8f]"
                    }`}
                  >
                    <svg
                      className="h-3.5 w-3.5 shrink-0"
                ...
                >
                  ...
                  >
                    <span
                      className={`block h-[2px] w-[20px] bg-current rounded-none transition-all duration-400 ease-[cubic-bezier(0.22,1,0.36,1)] ${isDrawerOpen ? "translate-y-[7px] rotate-45" : ""}`}
                    ></span>
                    <span
                      className={`block h-[2px] w-[20px] bg-current rounded-none transition-all duration-200 ${isDrawerOpen ? "opacity-0 scale-x-0" : ""}`}
                    ></span>
                    <span
                      className={`block h-[2px] w-[20px] bg-current rounded-none transition-all duration-400 ease-[cubic-bezier(0.22,1,0.36,1)] ${isDrawerOpen ? "-translate-y-[7px] -rotate-45" : ""}`}
                    ></span>
      ...
      >
        <div
          className="absolute inset-0 bg-[#333333]/40 backdrop-blur-md transition-opacity duration-400"
        ...
        >
          ...
          <div className="flex flex-1 flex-col justify-start gap-7 pt-[35px]">
            <nav className="flex flex-col gap-4">
              {[
                ...
                },
                {
                  label: "Wydarzenia",
                  href: "/wydarzenia",
                  hoverClass: "hover:text-[#fa8080]",
                },
                {
                  label: "Kontakt",
                  href: "/kontakt",
                  hoverClass: "hover:text-[#00955e]",
                },
              ].map((link, idx) => {
            ...
            <hr
              className={`border-[#333]/[0.07] ${isDrawerOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-[12px]"} transition-[opacity,transform] duration-400 ease-[cubic-bezier(0.22,1,0.36,1)]`}
            ...
            <div className="grid grid-cols-2 gap-8">
              <div>
                <p
                  className={`mb-4 px-1 text-[13px] font-bold uppercase tracking-[0.22em] text-[#6b6b6b] ${isDrawerOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-[12px]"} transition-[opacity,transform] duration-400 ease-[cubic-bezier(0.22,1,0.36,1)]`}
                  ...
                  {oNas.map((item, idx) => {
                    ...
                    return (
                      <a
                        ...
                        href={item.href}
                        onClick={() => setIsDrawerOpen(false)}
                        className={`text-[18px] transition-all hover:text-[#2c5ea9] ${active ? "text-[#2c5ea9] font-bold" : "text-[#4b4b4b] font-medium"} ${isDrawerOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-[12px]"} transition-[opacity,transform] duration-400 ease-[cubic-bezier(0.22,1,0.36,1)]`}
                        style={{ transitionDelay: `${0.35 + idx * 0.02}s` }}
                      >
                        {item.label}
                      </a>
                    );
