...
export const POST: APIRoute = async ({ request }) => {
      ...
      return jsonResponse(
        {
          success: false,
          error: "csrf",
          message: "Niedozwolone żądanie (CSRF).",
        },
        403,
      );
    }
  ...
  const reservationDate =
    formData.get("reservationDate")?.toString().trim() ?? "";
  const reservationTime =
    formData.get("reservationTime")?.toString().trim() ?? "";
    ...
    return jsonResponse(
      {
        ...
        error: "conflict",
        message:
          "Ten stolik jest już zajęty w wybranym terminie. Wybierz inny stolik lub zmień godzinę.",
        tableId,
  ...
  try {
    ...
    const isUnique =
      error?.code === "23505" ||
      String(error?.constraint ?? "").includes("table_slot");
    if (isUnique) {
      return jsonResponse(
        {
          success: false,
