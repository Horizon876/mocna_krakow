...
export async function ensureCafeGallerySeeded(): Promise<void> {
  const existing = await db.select({ id: cafePhotos.id }).from(cafePhotos).limit(1);
...
export async function getCafeGalleryPhotos(): Promise<Array<{ src: string; alt: string }>> {
  await ensureCafeGallerySeeded();
  const rows = await db.select().from(cafePhotos).orderBy(asc(cafePhotos.createdAt));
  return rows.map((p) => ({
    src: p.imageUrl,
