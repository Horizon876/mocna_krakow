...
async function sendEmail(to: string, subject: string, html: string, text: string) {
  ...
  if (subjectPrefix) {
    console.info(`[order-email] DEV: wysyłka przekierowana ${to} → ${recipient}`);
  }
...
function buildDeliveryBlock(data: OrderEmailData): string {
  ...
  const addressLine = data.shippingMethod !== 'paczkomat'
...
function getStatusConfig(status: string, data: OrderEmailData): StatusMessageConfig {
  switch (status) {
    case 'pending':
      return {
        subject: `Zamówienie złożone #${data.orderId.split('-')[0]} — MOCna!`,
    ...
    case 'paid':
      return {
        subject: `Zamówienie opłacone #${data.orderId.split('-')[0]} — MOCna!`,
    ...
    case 'confirmed':
      return {
        subject: `Zamówienie potwierdzone #${data.orderId.split('-')[0]} — MOCna!`,
    ...
    case 'processing':
      return {
        subject: `Zamówienie w realizacji #${data.orderId.split('-')[0]} — MOCna!`,
    ...
    case 'shipped':
      return {
        subject: `Zamówienie wysłane #${data.orderId.split('-')[0]} — MOCna!`,
    ...
    case 'cancelled':
      return {
        subject: `Zamówienie anulowane #${data.orderId.split('-')[0]} — MOCna!`,
    ...
    default:
      return {
        subject: `Aktualizacja zamówienia #${data.orderId.split('-')[0]} — MOCna!`,
          ...
          <tr>
            <td style="padding:28px 32px;">
              ...
              <p style="margin:0 0 8px;font-size:13px;color:#666;">
                ...
                <span style="color:#aaa;">nr #${data.orderId.split('-')[0]}</span>
              </p>
...
function buildOrderText(data: OrderEmailData, status: string): string {
  const cfg = getStatusConfig(status, data);
  const itemsLines = data.items.map(it => `  ${it.qty}× ${it.name} — ${it.price}`).join('\n');
  return [
    ...
    '',
    cfg.showDelivery ? `Dostawa: ${SHIPPING_LABELS[data.shippingMethod] ?? data.shippingMethod}` : '',
